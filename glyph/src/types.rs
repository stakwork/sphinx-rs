use anyhow::Result;
use rmp::{decode::bytes::Bytes, encode::buffer::ByteBuf};
...
fn serialize_controlmessage(buff: &mut ByteBuf, object: &ControlMessage) -> Result<()> {
    match object {
...
fn deserialize_controlmessage(bytes: &mut Bytes) -> Result<ControlMessage> {
...
fn serialize_controlresponse(buff: &mut ByteBuf, object: &ControlResponse) -> Result<()> {
    match object {
...
fn deserialize_controlresponse(bytes: &mut Bytes) -> Result<ControlResponse> {
    let peek = peek_byte(bytes)?;
    if peek == 0x81 {
        deserialize_map_len(bytes, 1)?;
    }
...
fn serialize_config(buff: &mut ByteBuf, object: &Config) -> Result<()> {
...
fn deserialize_config(bytes: &mut Bytes) -> Result<Config> {
...
fn serialize_wifiparams(buff: &mut ByteBuf, object: &WifiParams) -> Result<()> {
    serialize_map_len(buff, 2u32)?;
    serialize_string(buff, Some("ssid"), &object.ssid)?;
...
fn deserialize_wifiparams(bytes: &mut Bytes) -> Result<WifiParams> {
    deserialize_map_len(bytes, 2)?;
    let ssid = deserialize_string(bytes, Some("ssid"))?;
