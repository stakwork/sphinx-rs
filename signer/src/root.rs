...
fn handle_inner(
    ...
    mut bytes: Vec<u8>,
    do_log: bool,
) -> Result<(Vec<u8>, Mutations), VlsHandlerError> {
    ...
    if counter == u16::MAX {
        ...
        if counter + 1 != sequence || sequence == 0 {
            return Err(VlsHandlerError::BadSequence(sequence, counter + 1));
        } else {
...
pub fn handle_with_lss(
    ...
    bytes: Vec<u8>,
    do_log: bool,
) -> Result<(Vec<u8>, Vec<u8>), VlsHandlerError> {
