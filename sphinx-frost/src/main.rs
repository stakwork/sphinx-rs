...
fn main() {
    ...
    let frost2 = frost::new_with_deterministic_nonces::<Sha256>();
    ...
    let (shares_2, pop_2) = frost2.create_shares(&keygen, secret_poly_2);
    ...
    let (secret_share_2, frost_key2) = frost
    ...
    let frost_key2 = frost_key2.into_xonly_key();
    assert!(frost_key.public_key() == frost_key2.public_key());
    ...
    let mut nonce_rng: ChaCha20Rng =
        frost2.seed_nonce_rng(&frost_key2, &secret_share_2, session_id);
    let nonce_2 = frost2.gen_nonce(&mut nonce_rng);
    ...
    let sig_2 = frost2.sign(&frost_key2, &session2, 2, &secret_share_2, nonce_2);
